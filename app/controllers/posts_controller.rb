class PostsController < ApplicationController
  ...
  def index
   # @posts = Post.order('timestamp DESC').paginate(:page => params[:page], :per_page => 30)
    @posts = Post.where("bedrooms" => 100) if params["bedrooms"].present?
  ...
  private
    ...
    def post_params
       params.require(:post).permit(:heading, :body, :price, :neighborhood, :external_url,:bedrooms,:bathrooms, :sqft, :cats, :dogs ,:w_d_in_unit, :street_parking, :photo)
    end
